<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input v-model="listQuery.name" :placeholder="$t('table.applyName')" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter" />
    ...
    >
      ...
      <el-table-column v-if="showReviewer" :label="$t('table.reviewer')" width="110px" align="center">
        <template slot-scope="{row}">
          <span style="color:red;">{{ row.examine }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column :label="$t('table.applyAssociation')" width="110px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.applyAssociation }}</span>
        </template>
      </el-table-column>
...
<!--          &lt;!&ndash;&lt;!&ndash;          <el-button v-if="row.status!='draft'" size="mini" @click="handleModifyStatus(row,'draft')">&ndash;&gt;&ndash;&gt;-->
<!--          &lt;!&ndash;&lt;!&ndash;            {{ $t('table.draft') }}&ndash;&gt;&ndash;&gt;-->
<!--          &lt;!&ndash;&lt;!&ndash;          </el-button>&ndash;&gt;&ndash;&gt;-->
<!--          <el-button v-if="row.status!='deleted'" size="mini" type="danger" @click="handleRefuse(row,$index)">-->
<!--            淘汰面试-->
<!--          </el-button>-->
<!--          <el-button v-if="row.status!='deleted'" size="mini" type="success" @click="handlePass(row,$index)">-->
<!--            通过面试-->
<!--          </el-button>-->
<!--        </template>-->
<!--      </el-table-column>-->
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" width="70%">
      <el-form ref="dataForm" :rules="rules" :model="list" label-position="left" label-width="80px" style="width: 750px; margin-left:50px;">
        <el-form-item label="申请社团" style="width: 300px" >
          <el-input v-model="temp.applyAssociation"  :disabled="true" />
        </el-form-item>
        <!--        <el-form-item :label="$t('table.date')" prop="timestamp">-->
        <!--          <el-date-picker v-model="temp.timestamp" type="datetime" placeholder="Please pick a date" />-->
        <!--        </el-form-item>-->
        <el-form-item :label="$t('table.name')" style="width: 300px" >
        ...
        <el-form-item :label="$t('table.remark')">
          <el-input v-model="temp.introduce"  :rows="7" type="textarea"  :disabled="true" />
        </el-form-item>
      </el-form>
...
<script>
import { queryHistoryMember,queryMember,updateStatus,getHistoryAllApplyList } from '@/api/apply'
...
export default {
  ...
  data() {
    return {
      ...
      calendarTypeOptions,
      sortOptions: [{ label: '升序', key: '1' }, { label: '降序', key: '0' }],
  ...
  methods: {
    ...
    handleFilter() {
      ...
      queryHistoryMember(this.listQuery).then(response => {
        this.list = response.data
        //this.total = response.data.total
        this.total = response.count
        this.ass = response.ass
