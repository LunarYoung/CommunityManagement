...
<!--      </el-select>-->
<!--      &lt;!&ndash;      <el-select v-model="listQuery.sort" style="width: 140px" class="filter-item" >&ndash;&gt;-->
...
<!--      </el-button>-->
    ...
    >
      <el-table-column label="讨论ID" prop="id"  align="center" width="50" :class-name="getSortClass('id')">
        <template slot-scope="{row}">
          <span>{{ row.cid }}</span>
        </template>
      </el-table-column>
      <el-table-column label="评论用户名" min-width="100px">
        <template slot-scope="{row}">
      ...
      <el-table-column label="帖子内容" width="150px" align="center">
        <template slot-scope="{row}">
          <!--          <span>{{ row.timestamp | parseTime('{y}-{m}-{d} {h}:{i}') }}</span>-->
          <span>{{ row.content1}}</span>
        </template>
      </el-table-column>
      <el-table-column label="作者名称" min-width="100px">
        <template slot-scope="{row}">
          <span class="link-type" >{{ row.name1 }}</span>
          <!--          <el-tag>{{ row.type | typeFilter }}</el-tag>-->
        </template>
      </el-table-column>
...
import {
  ...
  upCAMstatus, CommentManageDus, upAADstatus
...
export default {
  ...
  methods: {
    getList() {
      ...
      CommentManageDus(this.page).then(response => {
        this.list = response.dusAndComment
