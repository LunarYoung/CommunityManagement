<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input v-model="listQuery.name" :placeholder="$t('table.memberName')" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter" />
...
<!--      </el-checkbox>-->
    ...
    >
      ...
      <el-table-column :label="$t('table.feedbackTime')" width="150px" align="center">
        <template slot-scope="{row}">
<!--          <span>{{ row.timestamp | parseTime('{y}-{m}-{d} {h}:{i}') }}</span>-->
         <span>{{ row.time}}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('table.memberName')" min-width="150px">
        <template slot-scope="{row}">
...
<!--          <el-tag>{{ row.type | typeFilter }}</el-tag>-->
      ...
      <el-table-column :label="$t('table.position')" width="80px">
        <template slot-scope="{row}">
          <span>{{ row.position }}</span>
<!--          <svg-icon v-for="n in +row.importance" :key="n" icon-class="star" class="meta-item__icon" />-->
        </template>
      </el-table-column>
      <el-table-column label="照片" align="center" width="95">
        <template slot-scope="{row}">
          <el-image
...
<script>
import {MemberList,QueryMember,deleteMemberById,updateMemberById} from '@/api/Association'
import waves from '@/directive/waves' // waves directive
import { parseTime } from '@/utils'
import Pagination from '@/components/Pagination' // secondary package based on el-pagination

const calendarTypeOptions = [
  { key: '部长', display_name: '1' },
  { key: '干事', display_name: '2' },
  { key: '社员', display_name: '3' }
]
...
export default {
  ...
  methods: {
    getList() {
      ...
      MemberList(this.page).then(response => {
